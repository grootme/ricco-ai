"""
Engram VCS - Version Control System para Memoria

Sistema de memoria con control de versiones basado en SQLite y FTS5.
"""
...
class DisclosureLevel(str, Enum):
    ...
    Protege el contexto del agente revelando solo la información necesaria.
    """
    COMPACT = "compact"      # Nivel 1: Solo IDs y relevancia
    TIMELINE = "timeline"    # Nivel 2: Línea temporal cronológica
    FULL = "full"           # Nivel 3: Contenido completo
...
class MemoryVCS:
    """
    Sistema de Memoria con Control de Versiones.
    ...
    def __init__(
        self,
        db_path: str = "~/.ricco-ai/engram.db",
        auto_init: bool = True
    ):
        """
    ...
    def _init_db(self) -> None:
        ...
        # Tabla de relaciones (grafo de conocimiento)
        cursor.execute('''
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_memories_cognitive_value ON memories(cognitive_value DESC)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_memory_versions_memory ON memory_versions(memory_id, version DESC)')
        
        conn.commit()
        conn.close()
        logger.info(f"Engram database initialized at {self.db_path}")
    ...
    def get_by_key(self, topic_key: str) -> Optional[Dict[str, Any]]:
        ...
        cursor.execute('SELECT * FROM memories WHERE topic_key = ?', (topic_key,))
        row = cursor.fetchone()
        conn.close()
