...
@pytest.fixture
def mock_interrupt():
    """Mock interrupt() to return a test user response."""
    with patch("deerflow.agents.middlewares.clarification_middleware.interrupt") as mock:
        mock.return_value = "user test response"
        yield mock
...
class TestClarificationCommandIdempotency:
    ...
    def test_repeated_tool_call_uses_stable_message_id(self, middleware, mock_interrupt):
    ...
    def test_missing_tool_call_id_still_gets_stable_message_id(self, middleware, mock_interrupt):
        request = SimpleNamespace(
            tool_call={
                "name": "ask_clarification",
                "args": {
